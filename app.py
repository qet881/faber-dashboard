...
def _load_hybrid_data(start_date, end_date):
    """하이브리드 로딩: ETF 상장 전은 프록시, 상장 후는 실제 ETF 데이터."""
    all_data = {}
    ...
    for name, ticker in ASSETS.items():
        ...
        # 가격 데이터: 체인링크
        all_data[name] = _chain_link_series(proxy_df, etf_df)
        
        # 모멘텀 데이터
        if ticker == '411060':
            # 금현물: 모멘텀은 항상 GLD×환율 (실전과 동일한 설계)
            etf_momentum = fetch_etf_data(ticker, start_date, end_date, is_momentum=True)
            all_data[f"{name}_모멘텀"] = _chain_link_series(proxy_df, etf_momentum)
        else:
            # 나머지: 가격 데이터와 동일
            all_data[f"{name}_모멘텀"] = all_data[name]
    
    # 현금: 프록시(합성) + 실제 MMF ETF 체인링크
    proxy_cash = fetch_proxy_data(CASH_NAME, start_date, end_date)
    etf_cash = fetch_etf_data(CASH_TICKER, start_date, end_date, is_momentum=False)
...
""")
    ...
    with st.expander("📊 데이터 가용 기간 확인 (하이브리드)"):
        for name in list(ASSETS.keys()) + [CASH_NAME]:
            ...
            note = ""
            if name in PROXY_ASSETS: note = f" ({PROXY_ASSETS[name]['note']} → 실제ETF)"
            elif name == CASH_NAME: note = f" ({PROXY_CASH['note']} → 실제MMF)"
