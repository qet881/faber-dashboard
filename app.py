...
def _is_month_end_rebalance_day(trading_dates, idx):
    ...
    if trading_dates is None or idx < 0 or idx >= len(trading_dates) - 1:
        return False
    d = trading_dates[idx]
...
def _collect_month_end_dates(trading_dates):
    ...
    if trading_dates is None or len(trading_dates) < 2:
        return []
    return [d for i, d in enumerate(trading_dates[:-1]) if _is_month_end_rebalance_day(trading_dates, i)]
